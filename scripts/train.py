...
parser.add_argument('--lambda',default=10.0,type=float,help='weight of cycle loss')
...
test_dataloader = DataLoader(MyDataset(dataname=args.dataset, phase='test'), batch_size=1)
...
for epoch in range(args.n_epochs):
    for i, batch in enumerate(train_dataloader):
        ...
        if i % 100 == 99:
            ...
            print('[%d/%d][%d/%d]\tLoss_G: %.4f\tLoss_D: %.4f\tD(G(x)): %.4f\t'
                        % (epoch+1, args.n_epochs, i+1, len(train_dataloader),
                            loss_gen.item(), (loss_D_A.item()+loss_D_B.item())/2, D_x))